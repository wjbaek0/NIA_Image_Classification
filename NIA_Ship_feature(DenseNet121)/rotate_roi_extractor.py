...
def parse_json_and_get_box_image(json_path, img_path):
    # crop rotated box images using json
    ...
    expand_img = np.zeros(
        (
            img.shape[0] + offset[0] + offset[1], 
            img.shape[1] + offset[0] + offset[1], 
            3
        ), 
        dtype=img.dtype
    )
    expand_img[
        offset[0]:img.shape[0] + offset[0], 
        offset[1]:img.shape[1] + offset[1]
    ] = img

    with open(json_path) as json_file:
        contents = json.load(json_file)

    for box_info in contents['Boxes']:
        rect = (
            (
                offset[1] + int(box_info['Rectangle']['X']) + int(box_info['Rectangle']['Width'] / 2.0), 
                offset[0] + int(box_info['Rectangle']['Y']) + int(box_info['Rectangle']['Height'] / 2.0)
            ),
            (
                int(box_info['Rectangle']['Width']), 
                int(box_info['Rectangle']['Height'])
            ),
            360.0 - box_info['Angle']
        )
        
        crops.append(crop_rotated_rectangle(expand_img, rect))
